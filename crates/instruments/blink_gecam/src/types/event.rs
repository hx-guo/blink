...
impl<S: Satellite> blink_core::traits::Event for Event<S> {
    ...
    ///   哨兵值、`E_MIN` 逐探头逐增益各不相同，能量无从谈起。**实测这一条
    ///   一票都没否过**：`EVT_TYPE == 1` 的事例里 `PI >= 448` 占 0.00%
    ///   （GECAM-A 25 路 × 3 个过境无一例外），溢出段与 `EVT_TYPE == 2` 是
    ///   同一个集合。留着是因为两者一个来自标志位、一个来自能标，任一处出
    ///   岔子另一处还能兜住。
    ///
    ///   **但 448 不是真正的量程上限。** 实测低增益支路的高能端在
