...
**这个脚本复刻的是 2026-09-11 修复之前的那道闸**（梯长写死 448、能阈道写死 54），
留着是因为它是"被拒的小时分别死在哪一条"的原始记录。修复之后加载器的规则不同了
（梯长与能阈道都从 EBOUNDS 自己推，见 OPEN-QUESTIONS 第 21 条），**要点当前
加载器的行为请用 `gc_ladder_rows.py`，不要改这个脚本去追**——追了就没有旧账了。
